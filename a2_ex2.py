...
def prepare_image(image: np.ndarray, x: int, y: int, width: int, height: int, size: int) -> \
    ...
    target_array_with_padding = np.zeros_like(image)
    target_array_with_padding[area] = target_array
    
    return pixelated_image, known_array, target_array, target_array_with_padding
